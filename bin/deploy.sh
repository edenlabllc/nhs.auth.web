...
#get version and project name
			...
			sed -i'' -e "15,20s/tag:.*/tag: \"$PROJECT_VERSION\"/g" "$Chart/values.yaml"
			helm init --upgrade
			sleep 15
			helm upgrade  -f $Chart/values.yaml  $Chart $Chart 
			cd $TRAVIS_BUILD_DIR/bin
			./wait-for-deployment.sh auth $Chart 180
   				if [ "$?" -eq 0 ]; then
     				kubectl get pod -n$Chart | grep auth 
     				cd $TRAVIS_BUILD_DIR/ehealth.charts && git add . && sudo  git commit -m "Bump $Chart auth to $PROJECT_VERSION" && sudo git pull && sudo git push
     				exit 0;
   				else 
   	 				kubectl logs $(sudo kubectl get pod -n$Chart | awk '{ print $1 }' | grep auth) -n$Chart 
